...
def create(self, username, machine_name, image, network, txn_id):
    ...
    logger.info('Task starting')
    try:
        resp['content'] = vmware.create_cee(username, machine_name, image, network, logger)
    except ValueError as doh:
        logger.error('Task failed: {}'.format(doh))
        resp['error'] = '{}'.format(doh)
    logger.info('Task complete')
...
def delete(self, username, machine_name, txn_id):
    ...
    logger.info('Task starting')
    try:
        vmware.delete_cee(username, machine_name, logger)
    except ValueError as doh:
        logger.error('Task failed: {}'.format(doh))
        resp['error'] = '{}'.format(doh)
    else:
...
@app.task(name='cee.modify_network', bind=True)
def modify_network(self, username, machine_name, new_network, txn_id):
    """Change the network a CEE instance is connected to"""
    logger = get_task_logger(txn_id=txn_id, task_id=self.request.id, loglevel=const.VLAB_CEE_LOG_LEVEL.upper())
    resp = {'content' : {}, 'error': None, 'params': {}}
    logger.info('Task starting')
    try:
